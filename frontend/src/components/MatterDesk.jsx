  ...
  return (
    <div className="matter-desk" data-testid="matter-desk">
      <div className="desk-page">
        ...
        <div className="desk-heading-row">
          <div>
            <div className="desk-eyebrow">
              THE BIG PICTURE. THE NEXT DECISION.
            </div>
            <h1>Your matters, in motion.</h1>
            <p>A clear path from open questions to considered advice.</p>
          </div>
          <button
        ...
        <div className="desk-section-title">
          <div>
            <h2>
              Your matters <span>{state.matters.length}</span>
            </h2>
            <p>Focus on the work that needs your judgment.</p>
          </div>
          <span className="desk-dataset">FICTIONAL PORTFOLIO / SEP 2026</span>
        </div>
        <div className="desk-filters">
          <label>
            ...
            <input
              ...
              placeholder="Search matters, clients, or ID…"
              value={query}
        ...
        <div className="desk-matters">
          {visible.map((m, index) => {
            const r = readiness(m);
            const n = m.issues.filter((i) => i.review).length;
            return (
              <button
                key={m.id}
                className={`desk-matter-card ${m.id === matter.id ? "selected" : ""}`}
                onClick={() => {
                  dispatch({ type: "select", matterId: m.id });
                  setSelectedIssue(m.issues[0].id);
                  setNotice("");
                }}
                aria-pressed={m.id === matter.id}
              >
                <span className="desk-card-meta">
                  {m.id}{" "}
                  <span className={`desk-practice tone-${index}`}>
                    {m.practice}
                  </span>
                </span>
                <strong>{m.name}</strong>
                <span className="desk-client">{m.client}</span>
                <div className="desk-card-progress">
                ...
                <div className="desk-track">
                  <i style={{ width: `${(n / m.issues.length) * 100}%` }} />
                </div>
        ...
        >
          ...
          <div className="desk-matter-details">
            ...
            <span>
              <Clock3 size={15} />
              Review target <b>{targetDate(matter.deadline) || "Set target"}</b>
            </span>
            <label>
              ...
              <div className="desk-review-layout">
                <div className="desk-issue-list">
                  ...
                  <div className="desk-queue-note">
                    <ShieldCheck size={17} />
                    <p>
                      Record progress at any stage. Evidence gaps remain visible
                ...
                <article className="desk-evidence">
                  ...
                  {issue.quote ? (
                    ...
                    <div className="desk-source missing">
                      ...
                      <p>
                        Assign research or explicitly exclude the unsupported
                        proposition. A demo action cannot verify real legal
                        authority.
                      </p>
                    </div>
                  )}
                  ...
                  {issue.pendingChange && (
                    <div className="desk-change">
                      ...
                      {issue.pendingChange.previousReview && (
                        <p>
                          Earlier review by{" "}
                          {issue.pendingChange.previousReview.reviewer} is no
                          longer current. Decision history is retained.
                        </p>
                      )}
                      <button
                  ...
                  <div className="desk-review-form">
                    ...
                    <label>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          dispatch({ type: "noteDraft", value: e.target.value })
                        }
                      />
                    </label>
            ...
            {view === "drafts" && (
              <div className="desk-drafts">
                <div className="desk-panel-intro">
                  <div>
                    <span className="desk-eyebrow">CONNECTED WORK PRODUCT</span>
                    <h3>Follow each statement to its source.</h3>
                    <p>
                      Edits and sample source changes reopen affected reviews
                      and revoke internal approval.
                    </p>
                  </div>
                  <button
                    className="desk-button"
                ...
                <div className="desk-draft-layout">
                    ...
                    {matter.issues.map((i) => (
                      <div className="desk-draft-statement" key={i.id}>
                        <div>
                          <span>{i.id}</span>
                          <span
                            className={i.review ? "desk-checked" : "desk-amber"}
                          >
                            {i.pendingChange
                              ? "Source changed"
                              : i.excluded
                  ...
                  <aside className="desk-dependencies">
                    ...
                    <p>
                      Source → finding → working statement → internal approval
                    </p>
                    {matter.issues.map((i) => (
                      <div key={i.id}>
                        <strong>
                        ...
                        {i.sources.length ? (
                          ...
                          <span className="desk-amber">No linked source</span>
                        )}
                        <small>
                          {i.pendingChange
                            ? "Source changed · Review reopened"
                    ...
                    <p className="desk-form-hint">
                      Links and comparisons are scripted for fictional matters.
                      No AI extraction runs.
                    </p>
                  </aside>
                </div>
                <section className="desk-import">
                  <span className="desk-eyebrow">INDEPENDENT DRAFT INTAKE</span>
                  <h3>Bring work from another tool.</h3>
                  <p>
                    Paste fictional text with references such as [N1 v3].
                    Inspection matches identifiers and versions in this matter;
                    it does not verify legal support or treatment.
                  </p>
                  <div className="desk-task-fields">
                    <label>
                      Draft origin
                  ...
                  {matter.imported && (
                    <div className="desk-import-results">
                      <h4>Reference inspection · {matter.imported.provider}</h4>
                      {matter.imported.outdated && (
                        <p className="desk-alert">
                          Sources changed after inspection. Inspect this draft
                          again.
                        </p>
                      )}
                      ...
                      <small>
                        No assertions are automatically approved or added to the
                        working memorandum.
                      </small>
                    </div>
                  )}
              ...
              <div className="desk-handoff">
                ...
                <div className="desk-handoff-columns">
                  ...
                  <div className="desk-prerequisites">
                    ...
                    <p>
                      Actions record the simulated participant and time. They do
                      not grant access or obtain client consent.
                    </p>
                    ...
                    {!state.actor.endsWith("Partner") && (
                      <small>Switch to the simulated partner to approve.</small>
                    )}
                    {matter.approval && (
                    ...
                    <button className="desk-button secondary" onClick={memo}>
                ...
                <section className="desk-deadline">
                  <span className="desk-eyebrow">DEPENDENCIES & TIMING</span>
                  ...
                  <p>
                    <b>Dependencies:</b> complete packet → resolve exceptions →
                    attorney review → partner approval. A target date does not
                    clear outstanding dependencies.
                  </p>
                  <button
                    className="desk-button secondary"
                    onClick={() =>
              ...
              <div className="desk-history">
                <div className="desk-panel-intro">
                  <div>
                    ...
                    <p>
                      Local, editable history with simulated participants. No
                      external notifications or immutable audit service.
                    </p>
                  </div>
              ...
              <div className="desk-value">
                <div>
                  ...
                  <p>
                    ...
                    attorney review. Editable assumptions, not measured savings
                    or a billing recommendation.
                  </p>
