...
test("opens on the clearly labeled Firm Operations portfolio surface", async ({
  page,
}) => {
  ...
  await expect(
    commandCenter.getByRole("heading", {
      name: "Law Suite Firm Operations",
      level: 1,
    }),
  ).toBeVisible();
...
test("filters adoption and access data and explains a governance signal", async ({
  page,
}) => {
  ...
  const adoptionSection = page
    .locator("section")
    .filter({ hasText: "Adoption by practice" });
  await expect(adoptionSection).toContainText("Tax");
  const trajectorySection = page
    .locator("section")
    .filter({ hasText: "Adoption trajectory" });
  await expect(trajectorySection).toContainText("30 days");

  const intelligenceSection = page
    .locator("section")
    .filter({ hasText: "Deployment intelligence" });
  await intelligenceSection
    .getByRole("button", { name: "Where is governance exposed?" })
    .click();
  await expect(
    intelligenceSection.getByRole("heading", {
      name: "Contractor access is the immediate control gap",
    }),
  ).toBeVisible();
  ...
  const accessSection = page
    .locator("section")
    .filter({ hasText: "Identity and access" });
  await accessSection
    .getByPlaceholder("Search people or practices")
    .fill("Sophie");
  await expect(
    accessSection.getByRole("row", { name: /Sophie Laurent/ }),
  ).toBeVisible();
  await expect(
    accessSection.getByRole("row", { name: /Amara Okafor/ }),
  ).toHaveCount(0);
  ...
  await expect(
    accessSection.getByRole("row", { name: /Emeka Balogun/ }),
  ).toBeVisible();
  await expect(
    accessSection.getByRole("row", { name: /James Wright/ }),
  ).toBeVisible();
});

test("turns a recommendation into an action and exports the adoption report", async ({
  page,
}) => {
  const recommendation = page
    .locator("article")
    .filter({ hasText: "Launch a Tax workflow clinic" });
  await recommendation.getByRole("button", { name: "Start plan" }).click();
  await expect(
    recommendation.getByRole("button", { name: "Plan started" }),
  ).toBeDisabled();
  await expect(
    page.getByText("Action plan started: Launch a Tax workflow clinic"),
  ).toBeVisible();
  ...
  expect(download.suggestedFilename()).toBe(
    "law-suite-firm-operations-adoption-report.csv",
  );
...
test("exports the selected practice and records a sample access decision", async ({
  page,
}) => {
  await page.getByLabel("Practice group filter").selectOption("Tax");
  const downloadPromise = page.waitForEvent("download");
  ...
  await expect(
    row.getByRole("button", { name: "Review noted (demo)" }),
  ).toBeDisabled();
});
